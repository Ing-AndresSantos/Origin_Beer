...
function exportPDF() {
    ...
    doc.setFontSize(26); doc.setTextColor(245, 158, 11);
    doc.text('🍺 Origin Beer', 148, 55, { align: 'center' });
    doc.setFontSize(14); doc.setTextColor(229, 231, 235);
    doc.text(`Sales Report — ${userBranchName}`, 148, 68, { align: 'center' });
    doc.setFontSize(10); doc.setTextColor(156, 163, 175);
    doc.text(`Cashier: ${u.firstName || ''} ${u.lastName || ''}  |  ${new Date().toLocaleString('es-CO')}`, 148, 80, { align: 'center' });
    ...
    kpis.forEach(([label, val]) => {
        ...
        doc.roundedRect(kx, ky, 60, 20, 3, 3, 'F');
        doc.setFontSize(8);  doc.setTextColor(156, 163, 175); doc.text(label, kx + 4, ky + 7);
        doc.setFontSize(11); doc.setTextColor(245, 158, 11);  doc.text(String(val), kx + 4, ky + 15);
        kx += 68;
    });
